class Solution:
    ...
    def lineOptions(self, i, board) -> set:
        options = set();
    ...
    def colOptions(self, i, board) -> set:
        options = set();
    ...
    def optionsOf(self, i, j ,board, all_options):
        lineExclusion = self.lineOptions(i, board)
        colExclusion = self.colOptions(j, board)
    ...
    def updatePaths(self,i_start ,j_start ,board, all_options):
        for i in range(len(board)):
            if not self.notEmpty(board[i][j_start]):
                options = self.optionsOf(i, j_start, board, all_options)
                if len(options) == 1:
        ...
        for j in range(len(board[i_start])):
            if not self.notEmpty(board[i_start][j]):
                options = self.optionsOf(i_start, j, board, all_options)
                if len(options) == 1:
        ...
        for i in range(len(board)):
            for j in range(len(board[i])):
                if not self.notEmpty(board[i][j]):
                    options = self.optionsOf(i, j, board, all_options)
                    if len(options) == 1:
    ...
    def solveEmpty(self, positions: List, all_options, board):
